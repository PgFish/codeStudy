<template>
  <div>
    ...
    <div class="index">
      <div class="header">
        <div class="share-shop-btn" @click="shareShop" v-if="!isZW">
          <img src="static/img/index/index_share.png" />
        </div>
        <div class="share-register" @click="inviteRegister">
          <img src="static/img/index/share_register.png" />
        </div>
        ...
        <div class="header-profile-box">
          ...
          <div class="user-info">
            ...
            <div class="go-withdraw" @click="toWithdraw">
              <img src="static/img/index/to_withdraw.png?v=1" />
            </div>
            <!-- <div class="nick-name">
          ...
          <div class="middle" @click="toOrderList">
            ...
            <template v-else>
              <div class="middle-item total-deal-finish">
      ...
      <div class="index-nav-items">
        <div class="nav-item" @click="toMoneyStrategy">
          <div class="nav-item-img">
            <img src="static/img/index/index_rule.png" />
            <div class="index-red-dot" v-if="ruleRedDot"></div>
          </div>
          <div class="nav-item-name">操作指南</div>
        </div>
        <div class="nav-item" @click="toIntro" v-if="!isZW">
          <div class="nav-item-img">
            <img src="static/img/index/index_introduce.png" />
            <div class="index-red-dot" v-if="introduceRedDot"></div>
          </div>
          <div class="nav-item-name">“合伙人”介绍</div>
        </div>
        <div class="nav-item" @click="toActivity" v-if="acts.length > 0">
          <div class="nav-item-img">
            <img src="static/img/index/index_activity.png" />
            <div class="index-red-dot" v-if="actRedDot"></div>
          </div>
          <div class="nav-item-name">活动</div>
        </div>
        <div class="nav-item" @click="toTask" v-if="!isZW && false">
          <div class="nav-item-img">
            <img src="static/img/index/index_task.png" />
          </div>
          <div class="nav-item-name">任务中心</div>
        </div>
      </div>
      ...
      <div class="category-list" v-if="list.length > 0" ref="scrollNav">
        ...
        <!-- <div :key="'category_llbg'" class="category-item" :class="{'category-item-selected': selectedCategory === 'llbg'}" @click="categoryChange('llbg')">
          <div class="category-divider" v-if="list.length > 0"></div>
          <div class="category-name" :class="{'is-ios': isIOS}">{{'流量白给'}}</div>
        </div> -->
      </div>
      <div class="fee-list">
        ...
        <!--<div v-show="'llbg' === selectedCategory">
          <div class="fee-item" :class="{'fee-item-1': list.length === 0}" :key="'category_llbg_fee_1'" @click="toPromiseFee">
            <div class="fee-img">
              <img src="static/img/index/llbg.png" />
            </div>
            <div class="fee-intro" style="width: calc(100% - 66px);margin-top: 6px;">
              <div class="fee-title">流量白给</div>
              <div class="fee-profile">最高可拿60G，连续白给15个月<br/>仅限成都用户办理</div>
              <div class="fee-share-profit"></div>
            </div>
          </div>
        </div> -->
      </div>
      ...
      <div class="ads-popup" v-if="showAds">
        <div class="ads-panel">
          <img :src="shopIndexAds.imgUrl" class="ads-img" @click.stop="toAdsURL(shopIndexAds)" />
...
<script>
import { closeIllegalPage, getSession, baseUrl, getRequestParams, sendGetRequest, sendJsonPostRequest, showMenuItems, saveStorage,
  getStorage, isWechatBoolean, handleShare, pageRouter, isWechat, sourceType, saveSession, encryptByDES, YEK_SED, isIOS } from 'src/assets/utils.js';
...
export default {
  ...
  created () {
    saveStorage('lastPartnerSystem', 'old');
    Indicator.close();
    document.title = '店铺';
    let that = this;
    ...
    */
    ...
    if (serverUserInfo) {
      ...
      this.invitationCode = serverUserInfo.invitationCode;
    ...
    saveStorage('20181114zt_intro_' + sourceId, '1'); // 从掌厅注册入口注册后进入首页成功
    saveStorage('shareshop_intro_' + sourceId, '1'); // 从店铺分享注册入口注册后进入首页成功
  ...
  methods: {
    async getshopIndexAds() {
      if (getSession('shop_index_ads')) {
        this.shopIndexAds = getSession('shop_index_ads', true);
        let hideIndexAds = getSession('hideIndexAds', true); // 本次关闭关注有礼活动的广告
        // 显示关注有礼活动的广告图弹层。从关注有礼跳转进入不显示、过期不显示、点击关闭按钮当前session不显示、装维合伙人不显示
        this.showAds = !/followreward/.test(this.$route.query.from) && !hideIndexAds && sourceType !== 'zhuangwei';
        return;
      }
      let url = `${baseUrl}/common/getSystemConfig`;
      let params = {
        'typeCode': 'SHOP_INDEX_POPUP'
      };
      let reqParams = getRequestParams(
        ...
        (vue, json) => {
          ...
          if (json && json.status === 0 && json.data.length > 0) {
            vue.shopIndexAds = json.data[0];
            if (vue.shopIndexAds) {
              let hideIndexAds = getSession('hideIndexAds', true); // 本次关闭关注有礼活动的广告
              // 显示关注有礼活动的广告图弹层。从关注有礼跳转进入不显示、过期不显示、点击关闭按钮当前session不显示、装维合伙人不显示
              vue.showAds = !/followreward/.test(vue.$route.query.from) && !hideIndexAds && sourceType !== 'zhuangwei';
            }
            saveSession('shop_index_ads', vue.shopIndexAds);
          }
        },
        (vue, ex) => {
          console.log(ex);
      ...
      await sendGetRequest(this, reqParams);
    },
    toAdsURL: function(item) {
      if (!item.value) return;
      this.closeAds();
      if (item.remark === 'router') {
        pageRouter(this, { path: item.value, query: { p: this.$route.query.p, ptid: this.$route.query.ptid } });
      } else if (/^http/i.test(item.value)) {
    ...
    shareShop: function() {
      ...
      sendGetRequest(this, reqParams);
      let shareUrl = `${window.location.origin + baseUrl}/common/redirect?info=path=shareshoplistenp=${phoneNumber}listenptid=${this.partnerId}listeninviteCode=${this.invitationCode}`;
      ...
      if (!isWechatBoolean) {
        ...
        let shareObj = {
          ...
          shareLink: shareUrl,
          sharePic: `${window.location.origin + baseUrl}/partner/static/img/cmcc_logo.png`
        };
    ...
    inviteRegister: function(e) {
      ...
      let obj = {
        ...
        link: `${window.location.origin + baseUrl}/campuspartner/pages/?pageName=registernew&inviteCode=${this.invitationCode}&from=old&noOldAuth=1`,
        imgUrl: `${window.location.origin + baseUrl}/partner/static/img/cmcc_logo.png`
      };
      if (!isWechatBoolean) {
        Indicator.close();
    ...
    // },
    toMoneyStrategy: function () {
      this.ruleRedDot = false;
      saveStorage('pt_shop_rule_red_dot_' + this.$route.query.p, false);
      pageRouter(this, { path: '/moneyStrategy', query: { p: this.$route.query.p, ptid: this.$route.query.ptid } });
    },
    toIntro: function () {
    ...
    jump: function (fee, categoryId) {
      Webtrends.multiTrack({ argsa: ['WT.nv', 'gpas_index_feeType', 'WT.event', 'gpas_feeId_' + fee.id] });
    ...
    share: function (e, fee, categoryId) {
      ...
      if ('' + categoryId !== '1' || fee.remark) { // 非0元号卡资费，跳转本地页面
        // shareUrl = this.buildInnerUrl(fee.id);
        // if (/^2|7|8$/.test(categoryId)) {
        ...
        if ('' + categoryId === '5') {
          ...
          if ('' + fee.deployMethod === '0') { // 自定义跳转
            if ('' + fee.remark === '1') { // 权益新升级
              shareUrl = this.buildCustomShareUrl('promisefee', fee.id);
            } else if ('' + fee.remark === '8') { // 电视业务
              shareUrl = this.buildCustomShareUrl('tvaddfee', fee.id);
            } else if ('' + fee.remark === '10') { // 聚合业务
              shareUrl = this.buildCustomShareUrl('consobusiness', fee.id);
            } else { // 未配置跳转方式，默认跳转通用资费模板（原特惠流量页面）
              shareUrl = this.buildCustomShareUrl('handlesuperflow', fee.id);
            }
          } else { // 后台配置，默认跳转通用资费模板（原特惠流量页面）
      ...
      if (/^2|7|8$/.test(categoryId) || /^\d+$/.test(fee.remark)) { // 2:特惠流量;7:玩出无穷新意;8:智慧生活
        shareTitle = fee.shareTitle || fee.productName;
        shareBrief = fee.shareBrief || fee.typeIntroduce;
      }
      let shareIcon = /^http/i.test(fee.icon) ? fee.icon : `${window.location.origin + baseUrl}/partner/static/img/cmcc_logo.png`;
      if (!isWechatBoolean) {
        Indicator.close();
        console.log(fee);
        let shareObj = {
    ...
    buildCustomShareUrl: function(pageName, id) {
      return `${window.location.origin + baseUrl}/common/redirect?info=path=${pageName}listenid=${id}listenpartnerId=${this.partnerId}`;
    },
    ...
    drawPoster: function () {
      ...
      if ('' + this.currentShareCategoryId !== '1' || this.currentShareFee.remark) { // 非0元号卡资费，跳转本地页面
        ...
        // if (/^2|7|8$/.test(this.currentShareCategoryId)) { // 热门业务/2:特惠流量;7:玩出无穷新意;8:智慧生活
        //   shareUrl = this.buildCustomShareUrl('handlesuperflow', this.currentShareFee.id);
        // } else
        if ('' + this.currentShareCategoryId === '5') {
          shareUrl = this.buildCustomShareUrl('familyDetail', this.currentShareFee.id);
        } else {
          if ('' + this.currentShareFee.deployMethod === '0') { // 自定义跳转
            if ('' + this.currentShareFee.remark === '1') { // 权益新升级
              shareUrl = this.buildCustomShareUrl('promisefee', this.currentShareFee.id);
              params = {
                qrText: shareUrl,
                qrX: 360 - 250 / 2 + 1,
                qrY: 357 + 1,
            ...
            } else if ('' + this.currentShareFee.remark === '8') { // 电视业务
              shareUrl = this.buildCustomShareUrl('tvaddfee', this.currentShareFee.id);
            } else if ('' + this.currentShareFee.remark === '10') { // 聚合业务
              shareUrl = this.buildCustomShareUrl('consobusiness', this.currentShareFee.id);
            } else { // 未配置跳转方式，默认跳转通用资费模板（原特惠流量页面）
              shareUrl = this.buildCustomShareUrl('handlesuperflow', this.currentShareFee.id);
            }
          } else { // 后台配置，默认跳转通用资费模板（原特惠流量页面）
