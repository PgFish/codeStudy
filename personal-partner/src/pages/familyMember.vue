  ...
  <div class="family-member">
    ...
    <div class="memberlist-section">
      <div class="memberlist-new-tip" v-if="pageType === 'new'">
        恭喜！您已成功开通合家欢业务，<br/>请添加合家欢群组成员。
      </div>
      <div class="memberlist-title">群组成员<span class="red-color">（最多可添加9人含召集人）</span></div>
      ...
      <div class="memberlist-content">
        <div class="memberlist-content-row flex-row">
          <div class="memberlist-content-row-phone">{{mainPhone}}<span class="" v-if="mainPhone === selfPhone">（我）</span></div>
        ...
        <div v-for="(item, index) in memberList" class="memberlist-content-row flex-row">
          <div class="memberlist-content-row-phone">{{item}}<span class="" v-if="item === selfPhone">(我)</span></div>
          <div class="memberlist-content-row-childtext">成员</div>
          <div class="memberlist-content-delete-btn" @click="deleteMember(item)" v-if="mainPhone === selfPhone">
            <img src="static/img/family/familyMember_delete.png" alt="">
          </div>
          <div class="memberlist-content-quit-btn" @click="quitMember(item)" v-if="mainPhone !== selfPhone">退出</div>
        </div>
      </div>
    </div>
    <div class="tab-wrap">
...
<script>
import { baseUrl, getRequestParams, sendGetRequest, Constants, sendJsonPostRequest, getJsApiConfig, isQQ, getSession, pageRouter } from 'src/assets/utils.js';
...
export default {
  ...
  methods: {
    ...
    getDetail: function() {
      ...
      let reqParams = getRequestParams(
        ...
        (vue, json) => {
          ...
          document.title = vue.mainPhone === vue.selfPhone ? '成员管理' : '成员列表';
          let shareObj = {
    ...
    addMember: function() {
      ...
      MessageBox.confirm(`您正在添加${this.childPhone}进入合家欢群组！`).then(action => {
        console.log(action);
        if (action === 'confirm') {
    ...
    quitMember: function(item) {
      MessageBox.confirm(`确认退出当前合家欢群组吗？`).then(action => {
        console.log(action);
        if (action === 'confirm') {
    ...
    quitMemberToServer: function(item) {
      ...
      let reqParams = getRequestParams(url, params, (vue, json) => {
        Indicator.close();
        if (json && ('' + json.status === '0')) {
          MessageBox.alert(`您已退出合家欢群组！`).then(action => {
            pageRouter(this, {path: '/familyDetail', query: {id: this.$route.query.id}});
          }).catch(ex => {
            pageRouter(this, {path: '/familyDetail', query: {id: this.$route.query.id}});
          });
    ...
    deleteMember: function(item) {
      MessageBox.confirm(`您正在从合家欢群组删除${item}！`).then(action => {
        console.log(action);
        if (action === 'confirm') {
