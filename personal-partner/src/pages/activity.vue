...
export default {
  ...
  created() {
    ...
    s.async = true;
    s.src = 'static/js/webtrends.load.js';
  ...
  methods: {
    ...
    goActivity: function(item) {
      ...
      if (item.remark !== 'native') {
        ...
        pageRouter(this, {
          path: item.value,
          query: { p: this.$route.query.p, ptid: this.$route.query.ptid }
        });
      }
    },
    toGivebackRule: function() {
      ...
      if (
        /192.168.|localhost|127.0.0.1/.test(prefix) ||
    ...
    toGiveback: function() {
      Webtrends.multiTrack({
        argsa: ['WT.nv', 'gpas_activity_actType', 'WT.event', 'gpas_giveback']
      });
      pageRouter(this, {
        path: '/giveback',
        query: { p: this.$route.query.p, ptid: this.$route.query.ptid }
    ...
    toChest: function() {
      ...
      Webtrends.multiTrack({
        argsa: ['WT.nv', 'gpas_activity_actType', 'WT.event', 'gpas_chest']
      });
      pageRouter(this, {
        path: '/chest',
        query: { p: this.$route.query.p, ptid: this.$route.query.ptid }
